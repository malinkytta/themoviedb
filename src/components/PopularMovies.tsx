...
const PopularMovies: React.FC<IProps> = ({ result, toggleText, toggle }) => {
    ...
    return (
        <>
            {result && (
                <>
                    <h2 className="py-4 popular-movies-title">Popular movies</h2>
                    ...
                    <Row className="py-5 pb-3 mb-4 mt-4 g-3 slider flex-nowrap ms-3 popular-movies">
                        {result.results.slice(0, 10).map(data => (
                            <Col key={data.id} className=" d-flex align-items-stretch">
                                <Card as={Link} to={`/popular/${data.id}`} className="movies-card">
